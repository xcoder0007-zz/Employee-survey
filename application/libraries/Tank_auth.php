...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->model('tank_auth/users_model');
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users_model->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							$this->ci->users_model->update_login_info(
									$user->id,
	...
	function demo_login() {
		$this->ci->session->set_userdata(array('user_id' => '1', 'username' => 'demo', 'status' => '1', 'is_admin' => '1'));
		redirect('/');
	}

	function qr_login() {
		$this->ci->session->set_userdata(array('user_id' => '1', 'username' => 'demo', 'status' => '1', 'is_admin' => '0'));
	...
	{
		if ((strlen($username) > 0) AND !$this->ci->users_model->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');

		} elseif (!$this->ci->users_model->is_email_available($email)) {
			...
			if (!is_null($res = $this->ci->users_model->create_user($data, !$email_activation))) {
	...
	{
		return ((strlen($username) > 0) AND $this->ci->users_model->is_username_available($username));
	}
	...
	{
		return ((strlen($email) > 0) AND $this->ci->users_model->is_email_available($email));
	}

	/**
	 * Change email for activation and return some data about user:
	 * user_id, username, email, new_email_key.
	 * Can be called for not activated users_model only.
	 *
	 * @param	string
	 * @return	array
	 */
	function change_email($email)
	{
		...
		if (!is_null($user = $this->ci->users_model->get_user_by_id($user_id, FALSE))) {
			...
			if (strtolower($user->email) == strtolower($email)) {		// leave activation key as is
				...
				$data['new_email_key'] = md5(rand().microtime());
				$this->ci->users_model->set_new_email($user_id, $email, $data['new_email_key'], FALSE);
				return $data;
	...
	{
		$this->ci->users_model->purge_na($this->ci->config->item('email_activation_expire', 'tank_auth'));

		if ((strlen($user_id) > 0) AND (strlen($activation_key) > 0)) {
			return $this->ci->users_model->activate_user($user_id, $activation_key, $activate_by_email);
		}
		return FALSE;
	}
	...
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users_model->get_user_by_login($login))) {
				...
				$this->ci->users_model->set_password_key($user->id, $data['new_pass_key']);
				return $data;
	...
	function can_reset_password($user_id, $new_pass_key)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0)) {
			return $this->ci->users_model->can_reset_password(
				$user_id,
				$new_pass_key,
	...
	function reset_password($user_id, $new_pass_key, $new_password)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users_model->get_user_by_id($user_id, TRUE))) {
				...
				if ($this->ci->users_model->reset_password(
						$user_id,
						$hashed_password,
						$new_pass_key,
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if (!is_null($user = $this->ci->users_model->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($old_pass, $user->password)) {			// success
				...
				$this->ci->users_model->change_password($user_id, $hashed_password);
				return TRUE;
	...
	function set_new_email($new_email, $password)
	{
		...
		if (!is_null($user = $this->ci->users_model->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				} elseif ($this->ci->users_model->is_email_available($new_email)) {
					$data['new_email_key'] = md5(rand().microtime());
					$this->ci->users_model->set_new_email($user_id, $new_email, $data['new_email_key'], TRUE);
					return $data;
	...
	function activate_new_email($user_id, $new_email_key)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_email_key) > 0)) {
			return $this->ci->users_model->activate_new_email(
					$user_id,
					$new_email_key);
		}
		return FALSE;
	...
	function delete_user($password)
	{
		...
		if (!is_null($user = $this->ci->users_model->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				$this->ci->users_model->delete_user($user_id);
				$this->logout();
				return TRUE;
			...
			if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
				...
				if (isset($data['key']) AND isset($data['user_id'])) {
					...
					if (!is_null($user = $this->ci->user_autologin->get($data['user_id'], md5($data['key'])))) {
						...
						// Renew users_model cookie to prevent it from expiring
						set_cookie(array(
								'name' 		=> $this->ci->config->item('autologin_cookie_name', 'tank_auth'),
								'value'		=> $cookie,
								'expire'	=> $this->ci->config->item('autologin_cookie_life', 'tank_auth'),
