    ...
    $.fn.filterTable = function(options) { // define the filterTable plugin
        ...
        var doFiltering = function(table, q, searchable) { // handle the actual table filtering
                ...
                if (q==='') { // if the filtering query is blank
                    ...
                    tbody.find(searchable).removeClass(settings.highlightClass).filter(':filterTableFind("'+q.replace(/(['"])/g,'\\$1')+'")').addClass(settings.highlightClass).closest('tr').show().addClass(settings.visibleClass).next().show().addClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
                }
        ...
        return this.each(function() {
            ...
            if (t[0].nodeName==='TABLE' && tbody.length>0 && (settings.minRows===0 || (settings.minRows>0 && tbody.find('tr').length>settings.minRows)) && !t.prev().hasClass(settings.containerClass)) { // only if object is a table and there's a tbody and at least minRows trs and hasn't already had a filter added
                if (settings.inputSelector && $(settings.inputSelector).length===1) { // use a single existing field as the filter input field
                    ...
                    filter = $('<input class="noprint" type="'+settings.inputType+'" placeholder="'+settings.placeholder+'" name="'+settings.inputName+'" />'); // build the filter field
